		...
		<table cellspacing="0" id="main-body" class="main">
			<tr>
				<td class="sidebar" nowrap="nowrap">
					...
					<li><a href="?page=<?= $page_lc ?>&type=logview">Log Viewer</a><br />
					...
					switch($page_lc)
					{
						...
						default:
							switch($type)
							{
								...
								case "logview"		:	$this->ViewLogs(); break;
								case "server_compile"		:	$this->CompileServer(); break;
								case "server_reboot"		:	$this->RebootLogin(); $this->RebootServer(); break;
								default				: 
	...
	*/
              ...
              private function RebootLogin()
	{
		...
		<table width="700" cellspacing="0" border="0">
			<tr>
				<td><h3>Using this form, you can queu a command to the EQ2DB<br>Server's cron to force a restart of the login server. Use with caution!</h3></td>
			</tr>
			<tr>
	...
	private function RebootServer()
	{
		...
		*/
		...
		<table width="700" cellspacing="0" border="0">
			<tr>
				<td><h4>World Server Reset</h4></td>
			</tr>
			<!-- <tr>
	...
	{
		$file_url = isset($_GET['file']) ? $_GET['file'] : null;

		$dir = "/home/eq2dev/eq2emu/logs/";
		...
		if (file_exists($dir) && is_dir($dir) ) {
			...
			if (strlen ( $file_url ) === 0 ) {
				...
				foreach ($files_arr as $file) {
					/* Only for loading login logs if needed.
		        	if (strpos($file, "eq2login") !== false) {
						?>
						<a href="index.php?page=admin&type=logview&file=<?php print(rtrim($file, ".log")) ?>"><?php print($file) ?></a><br/>
						<?php
						echo "<br/>";
						echo $file_url;
						echo "<br/>";
				...
				<h1>Viewing <?php print($file_url) ?></h1><br/>

				<textarea style="background-color: black;color:#3ba932;" rows="40" cols="95" wrap="off" style="overflow:scroll;">
				<?php
					$loadLog = sprintf("%s%s.log", $dir, $file_url);
					$myfile = fopen($loadLog, "r") or die("Unable to open file!");
					echo "&#10;";
					while(!feof($myfile)) {
						echo fgets($myfile);
					}
		    ...
		    echo "Directory does not exist or no files exist";
		}
	} // end of private function ViewLogs()
