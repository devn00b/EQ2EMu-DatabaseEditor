	...
	*/
	...
	private function RebootServer()
	{
		...
		if( strlen($_POST['force-restart']) > 0 ) {
			...
			$myfile = fopen($file, 'w') or die("Unable to open file!");
			$txt = "#!/bin/bash\n#\nrunning = true\necho \"'$running'\"\necho \"killall eq2world\"\necho \"Stopping eq2world...\" > /var/www/html/editors/eq2db2/reboot.log\nSleep 15\nrm -f /var/www/html/editors/eq2db2/force-restart\necho \"Resetting reboot command...\" >> /var/www/html/editors/eq2db2/reboot.log\necho \"Done. Please allow up to 5 minutes for full server boot up time.\" >> /var/www/html/editors/eq2db2/reboot.log\nexit\n";
			fwrite($myfile, $txt);
			fclose($myfile);
