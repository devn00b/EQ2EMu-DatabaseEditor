...
class eq2Admin
{
	...
	public function GetServerStats()
	{
		...
		if( is_array($tables) )
		{
			...
			{
				$server_stats[$table['TABLE_NAME']] .= $this->GetTotalRows(DEV_DB, $table['TABLE_NAME']);
			}
	...
	public function GetUserNameByID($id)
	{
		...
		$eq2->SQLQuery = sprintf("SELECT username FROM users WHERE id = %lu;", $_GET['id']);
		return $eq2->RunQuerySingle();
	}
	...
	private function CheckForSpawnIDConflict($spawn_id) 
	{
		...
		$result = $eq2->RunQuerySingle(sprintf("SELECT IF(s2.id IS NOT NULL AND s1.name != s2.name, 1, 0) AS 'CONFLICT' FROM %s.spawn s1 LEFT JOIN %s.spawn s2 ON s1.id = s2.id WHERE s1.id = %u", RAW_DB, DEV_DB, $spawn_id));
		...
		if ($result['CONFLICT'] == '1') {
			...
			$row = $eq2->RunQuerySingle(sprintf("SELECT max(id)+1 AS 'NEWID' FROM %s.spawn WHERE id BETWEEN %u0000 AND %u9999", DEV_DB, $range, $range));
			$old_id = $spawn_id;
			$spawn_id = $row['NEWID'];
	...
	private function MigrateSpawnData($spawn_id, $type = NULL)
	{
		...
		$eq2->SQLQuery = $eq2->GetRowCloneQuery(RAW_DB, "spawn", "id", $spawn_id, $spawn_id, NULL, DEV_DB);
		...
		$inserts = array();
		if (isset($type)) {
			$inserts[] = $eq2->GetRowCloneQuery(RAW_DB, "spawn_".$type, "spawn_id", $spawn_id, $spawn_id, "'id'", DEV_DB);
			if ($type == "npcs") {
				$inserts[] = $eq2->SQLQuery = $eq2->GetRowCloneQuery(RAW_DB, "npc_appearance", "spawn_id", $spawn_id, $spawn_id, "'id'", DEV_DB);
				$inserts[] = $eq2->SQLQuery = $eq2->GetRowCloneQuery(RAW_DB, "npc_appearance_equip", "spawn_id", $spawn_id, $spawn_id, "'id'", DEV_DB);
			}
		}
		else {
			...
			foreach ($SPAWN_TABLES as $k=>$v) {
				$inserts[] = $inserts[] = $eq2->GetRowCloneQuery(RAW_DB, $k, "spawn_id", $spawn_id, $spawn_id, "'id'", DEV_DB, true);
			}
		}
