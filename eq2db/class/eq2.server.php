...
class EQ2Server
{
	...
	private function GetOpcodeList($val = '')
	{
		...
		if( strlen($val) == 0 ) {
			...
			$eq2->SQLQuery = "SELECT * FROM ". DEV_DB .".opcodes WHERE `opcode` RLIKE '".$val."' OR `name` RLIKE '".$val."' ORDER BY name;";
	...
	public function GetNextEntityCommandListID() {
		...
		$data = $eq2->RunQuerySingle('SELECT MAX(`command_list_id`)+1 as nxt FROM `'.DEV_DB.'`.entity_commands;');
		return $data['nxt'];
	}
	...
	public function PrintStartingFactions() {
		...
		<fieldset><legend>Starting Factions</legend>
				...
				<?php endif; ?>
				<?php
				$query=sprintf("select * from %s.starting_factions order by starting_city, id", DEV_DB);
				$i = 0;
	...
	public function PreInsert() {
		...
		if ($page == "npc_spells") {
			...
			$query = sprintf("INSERT INTO %s.spawn_npc_spell_lists (`description`, `category`) VALUES ('%s','%s')", DEV_DB, $eq2->SQLEscape($desc), $eq2->SQLEscape($cat));
	...
	public function PostInsert($insert_res) {
		...
		if ($page == "groundspawns" && ($insert_res ?? 0) == 1) {
			...
			$eq2->RunQuery(false, sprintf('UPDATE %s.groundspawns SET groundspawn_id = %s WHERE id = %s', DEV_DB, $new_gs_id, $new_gs_id));
		}
			...
			if (isset($_GET['new'])) {
				...
				$query = sprintf("UPDATE %s.recipe_comp_list SET bEmpty = 0 WHERE id = %s", DEV_DB, $_GET['id']);
				$eq2->RunQuery(true, $query);
			}
		}
	...
	public function PrintRecipeComponentRow($comp, $table, $idField, $qtyField) {
		...
		$id = $comp[$idField] ?? 0;
		$qty = $comp[$qtyField] ?? 1;
		$query = sprintf("SELECT * FROM %s.recipe_comp_list WHERE id = %s", DEV_DB, $id);
	...
	function PrintRecipeForm($id) {
		...
		$query = sprintf("SELECT * FROM %s.recipe WHERE id = %s", DEV_DB, $id);
					...
					<td valign="top">
						...
						<fieldset style="white-space:nowrap">
							...
							<table class="RecipeDataTable">
								...
								<?php
								$query = sprintf("SELECT * FROM %s.recipe_secondary_comp WHERE recipe_id = %s ORDER BY `index`", DEV_DB, $id);
							...
							<table class="RecipeDataTable">
								...
								<table class="RecipeDataTable" valign="top">
									...
									<?php for ($x = 0; $x < 2; $x++) : ?>
										...
										<td align="center" style="border-right:2px solid #3b271c">
										<?php if ($r[$stageID] != 0) {
											$query = sprintf("SELECT name, icon, crafted, tier FROM %s.items WHERE id = %s", DEV_DB, $r[$stageID]);
											$data = $eq2->RunQuerySingle($query);
											printf("<table>");
	...
	function PrintCompListRow($item, $withForm = true) {
		...
		if ($id != null) {
			$data = $eq2->RunQuerySingle(sprintf("SELECT icon, tier, crafted, name FROM %s.items WHERE id = %s", DEV_DB, $id));
		}
?>
        <?php if ($withForm) { ?>
