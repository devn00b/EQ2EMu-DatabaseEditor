...
class eq2Spells
{
	...
	public function SaveReIndexedSpell()
	{
		...
		//$eq2->SQLQuery = sprintf("UPDATE ".DEV_DB.".item_details_skill SET spell_id = %s WHERE spell_id = %s", $_POST['next_id'], $_POST['old_id']);
		//$eq2->RunQuery();
		...
		if( $spell_id > 0 )
		{
			...
			if( is_array($spell_display_effects) )
			{
				...
				if( !empty($values) ) 
				{
					...
					$eq2->SQLQuery = sprintf("DELETE FROM ".DEV_DB.".spell_display_effects WHERE spell_id = %s and tier = %s;", $_GET['id'], $_GET['tier']);
					$eq2->RunQuery();
					
					$eq2->SQLQuery = sprintf("INSERT INTO ".DEV_DB.".spell_display_effects (spell_id, tier, description, bullet) VALUES %s;", $values); 
					$eq2->RunQuery();
...
//		// note: removed DEV_DB for logging purposes - do I even need it?
...
function CreateNewSpell() {
    ...
    $eq2->RunQuery(true, "LOCK TABLE ".DEV_DB."`spells` WRITE;");

    $query = sprintf('INSERT INTO %s.spells(name, type, description) VALUES("%s", "%s", "New spell.")', DEV_DB, $eq2->SQLEscape($name), $type);
    ...
    if ($success) {
        $id = $eq2->RunQuerySingle(sprintf("SELECT MAX(id) FROM %s.spells ORDER BY id DESC LIMIT 1;", DEV_DB))["MAX(id)"];
        $eq2->RunQuery(true, sprintf("INSERT INTO %s.spell_tiers(spell_id, tier) VALUES(%s, 1);", DEV_DB, $id));
