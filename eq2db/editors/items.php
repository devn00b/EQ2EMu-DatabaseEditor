...
switch(strtolower($_POST['cmd'] ?? "")) {
	...
	case "create":
		if ($_GET['show'] == "new") {
			...
			$query = sprintf("UPDATE %s.items SET `lua_script` = '%s' WHERE `id` = %s", DEV_DB, $eq2->db->sql_escape($scriptFile), $_GET['id']);
						...
						<form method="post" name="form2">
							...
							<tr>
								...
								<td colspan="2">
									...
									<?php									
									$query=sprintf("select distinct item_type from %s.items order by item_type", DEV_DB);
									$result=$eq2->db->sql_query($query);
									$t = $_POST['t'] ?? null;
									while($data=$eq2->db->sql_fetchrow($result)) {
