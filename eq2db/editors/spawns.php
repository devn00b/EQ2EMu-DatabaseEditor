...
if( ($_GET['search'] ?? "") != "" )
{
	...
	DisplaySpawnSelectionGrid($data);
	include("footer.php");
	exit; // end page here, since actions requires none of the code below
}
else if( strlen($spawns->spawn_type) > 0 && $spawns->spawn_id == 0 )
{
	...
	DisplaySpawnSelectionGrid($spawn_data);
}
...
function DisplaySpawnSelectionGrid($spawn_data)
{
	global $eq2, $spawns;
	...
	<table class="ContrastTable">
		<tr>
			<th width="96"><strong>Controls</strong></th>
			<th width="80"><strong>Spawn ID</strong></th>
			<th width="230"><strong>Name</strong></th>
			<?php
			switch($spawns->spawn_type)
			{
				case "npcs":
					?>
					<th width="350"><strong>SubTitle</strong></th>
					<th width="70"><strong>RaceID</strong></th>
					<th width="70"><strong>Model</strong></th>
					<th width="100"><strong>Min/Max/Enc</strong></th>
					<th width="120"><strong>Class</strong></th>
					<th width="70"><strong>Gender</strong></th>
					<?php
					break;
				...
				case "objects":
					...
					<th width="70"><strong>RaceID</strong></th>
					<th width="70"><strong>Model</strong></th>
					<th width="70"><strong>DeviceID</strong></th>
					<?php
					break;
					
				case "signs":
					?>
					<th width="70"><strong>Type</strong></th>
					<th width="250"><strong>Title</strong></th>
					<th width="350"><strong>Description</strong></th>
					<th width="70"><strong>Model</strong></th>
					<th width="70"><strong>ZoneID</strong></th>
					<?php
					break;
				...
				case "ground":
					?>
					<th width="70"><strong>GroundspawnID</strong></th>
				...
				default:
					...
					<th width="70"><strong>Model</strong></th>
					<th width="100"><strong>Min/Max/Enc</strong></th>
					<th width="70"><strong>Sign Type</strong></th>
					<th width="150"><strong>Sign Title</strong></th>
					<th width="50"><strong>Sign Zone</strong></th>
					<th width="70"><strong>GroundspawnID</strong></th>
					<th width="70"><strong>Skill</strong></th>
					<?php
					break;
		...
		foreach($spawn_data as $row) :
			$querystring = sprintf("spawns.php?zone=%s&type=%s&id=%s", 
														 ( $spawns->zone_id > 0 ) ? $spawns->zone_id : $row['loc_zone'],
														 ( strlen($spawns->spawn_type) > 0 ) ? $spawns->spawn_type : $row['spawn_type'],
														 ( $spawns->spawn_id > 0 ) ? $spawns->spawn_id : $row['id']);
		?>
		<tr align="center">
			<td>
				<form method="post" name="DeleteRow|<?=$i?>">
				<table>
					<tr>
				<td><a href="http://eq2.wikia.com/wiki/<?= $row['name'] ?>" target="_blank"><img src="../images/wikia.png" border="0" align="top" title="Wikia" alt="Wikia" height="16" /></a></td>
				<td><a href="http://eq2.zam.com/search.html?q=<?= $row['name'] ?>" target="_blank"><img src="../images/zam.png" border="0" align="top" title="Zam" alt="Zam" height="16" /></a></td>
				<?php if( $eq2->CheckAccess(G_DEVELOPER) ) : ?>
				<td><input id="Delete|<?= $i?>" value="Delete" name="cmd" type="image" class="SpawnDelete" src="../images/cross.png" border="0" align="top" title="Delete" alt="Delete" height="16" onclick="return confirm('Are you sure you want to delete this?');"/></input>
				<input type="hidden" name="orig_id" value="<?=$row['id']?>" />
				<input type="hidden" name="table_name" value="spawn" />
				</td>	
				<td><?php $eq2->GenerateBlueCheckbox("DeleteSpawn", false, "DeleteConfirm|".$i) ?></td>
				<?php endif; ?>
					</tr>
				</table>
				</form>
				<script>ElementToggleCheckbox("DeleteConfirm|<?=$i?>","Delete|<?= $i?>")</script>
			</td>
			<td><a href="<?= $querystring ?>"><?= $row['id'] ?></a></td>
			<td nowrap="nowrap">
				<?= $row['name'] ?>
			</td>
			<?php 
			switch($spawns->spawn_type)
			{
				case "npcs":
					?>
					<td nowrap="nowrap"><?= preg_replace("/<(.*)>/","&lt;$1&gt;", $row['sub_title']) ?></td>
					<td><?= $row['race']?></td>
					<td><?= $row['model_type']?></td>
					<td><?= $row['min_level']?>/<?= $row['max_level']?>/<?= $row['enc_level']?></td>
					<td nowrap="nowrap"><?= $eq2->eq2Classes[$row['class_']] ?></td>
					<td><?= $eq2->eq2Genders[$row['gender']] ?></td>
					<?php
					break;

				case "objects":
					?>
					<td><?= $row['race'] ? $row['race'] : "" ?></td>
					<td><?= $row['model_type'] ?></td>
					<td><?= $row['device_id'] ? $row['device_id'] : "" ?></td>
					<?php
					break;
				...
				case "ground":
					?>
					<td><?= $row['groundspawn_id'] ?></td>
					<td><?= $row['collection_skill'] ?></td>
					<?php
					break;
				...
				default:
					...
					<td nowrap="nowrap"><?= preg_replace("/<(.*)>/","&lt;$1&gt;", $row['sub_title']) ?></td>
					<td><?= $row['model_type']?></td>
					<td><?= ( $row['min_level'] ) ? sprintf("%s/%s/%s", $row['min_level'], $row['max_level'], $row['enc_level']) : "" ?></td>
					<td><?= $row['type'] ?></td>
					<td nowrap="nowrap"><?= $row['title'] ?></td>
					<td><?= $row['zone_id'] ? $row['zone_id'] : "" ?></td>
					<td><?= $row['groundspawn_id'] ? $row['groundspawn_id'] : "" ?></td>
					<td><?= $row['collection_skill'] ?></td>
					<?php
					break;
			}
	...
	<?php else : print("&nbsp;No data found for set filters. Try looking up the spawn by name.");
	endif;
}
...
function spawnlocation() 
{
	...
	?>
	<br />
	<table width="1100" cellpadding="5" border="0">
		<tr>
			<td>
				<span class="heading">Editing: <?= $spawns->spawn_name ?> (<?= $spawns->spawn_id ?>)</span>
				<br><br>
				<span style="font-size:14px;font-weight:bold;color:red">To create a new spawn location create a `spawn_location_entry` row followed by a `spawn_location_placement` below.</span>
				<br>
			</td>
		</tr>
		<tr>
		<td>
		<p>Jump to: <a href="#entry">spawn_location_entry</a> | <a href="#placement">spawn_location_placement</a></p>
		</td>
		</tr>
		<tr>
			<td valign="top">
				<?php $results = $spawns->GetSpawnLocationNames(); // moved here to get count() ?>
				<fieldset><legend>Table: spawn_location_name (<?php echo count($results) ?>)</legend>
				<table class="ContrastTable" cellpadding="0">
					<tr align="center">
						<th width="75">id</th>
				...
				if( is_array($results) )
				{
					...
					?>
					<tr align="center">
					<form method="post" name="zsgForm">
						<td>
							<strong><?php print($data['id']); ?></strong>
							<input type="hidden" name="orig_id" value="<?php print($data['id']); ?>" />
							<input type="hidden" name="object_id" value="<?= $spawns->spawn_name ?>|<?= $spawns->spawn_id ?>" />
							<input type="hidden" name="table_name" value="spawn_location_name" />
						</td>
						<td>
							<input type="text" name="spawn_location_name|name" value="<?php print($data['name']); ?>" style="width:300px;" />
	...
	<table width="1100" cellpadding="5" border="0">
		<tr>
			<td valign="top">
				<?php $results = $spawns->GetSpawnLocationEntries(); // moved here to get count() ?>
				<fieldset><legend>Table: spawn_location_entry (<?php echo count($results) ?>)</legend>
				<table class="ContrastTable" cellpadding="0">
					<tr align="center">
						<th colspan="5">
						</th>
						<th colspan="4">
							<strong style="color:red;text-decoration:underline">Conditionals</strong>
						</th>
						<th colspan="2">
						</th>
					</tr>
					<tr align="center">
						...
						<th width="105">spawn_id</th>
						<th width="105">spawn_location_id</th>
						<th width="105">spawnpercentage</th>
						<th width="45" style="color:red">Day</th>
						<th width="45" style="color:red">Night</th>
						<th width="45" style="color:red">Raining</th>
						<th width="45" style="color:red">No Rain</th>
						<th colspan="2">&nbsp;</th>
					</tr>
				<?php
					if( $eq2->CheckAccess(G_DEVELOPER) ) : ?>
						<tr align="center">
						<form method="post" name="zseForm|new">
							<td><strong>new</strong></td>
							<td><input type="text" name="spawn_location_name|name|new" value="" style="width:125px;" /></td>
							<td><strong><?php echo $_GET['id'] ?></strong></td>
							<td><strong>new</strong></td>
							<td><input type="text" name="spawn_location_entry|spawnpercentage|new" value="100" style="width:100px;" /></td>
							<?php 
								for ($i = 0; $i < 4; $i++) {
									$mask = 1 << $i;
									print("<td>");
									$eq2->GenerateBlueCheckbox("spawn_location_entry|condition|".$mask, false);
									print("</td>");
								}
							?>
							<td colspan="2">
								<input type="submit" value="Insert" style="font-size:10px; width:60px" />
								<input type="hidden" name="cmd" value="InsertSpawnLoc" />
							</td>
							</form>
						</tr>
					<?php endif;
				if( is_array($results) )
				{
					...
					?>
					<tr align="center">
					<form method="post" name="zseForm">
						<td>
							<strong><?php print($data['id']); ?></strong>
							<input type="hidden" name="orig_id" value="<?php print($data['id']); ?>" />
							<input type="hidden" name="object_id" value="<?= $spawns->spawn_name ?>|<?= $spawns->spawn_id ?>" />
							<input type="hidden" name="table_name" value="spawn_location_entry" />
						</td>
						<td>
							<span><?php echo $data['name'] ?></span>
						</td>
						<td>
							<input type="text" name="spawn_location_entry|spawn_id" value="<?php print($data['spawn_id']); ?>" style="width:100px;" />
							<input type="hidden" name="orig_spawn_id" value="<?php print($data['spawn_id']); ?>" />
						</td>
						<td>
							<input type="text" name="spawn_location_entry|spawn_location_id" value="<?php print($data['spawn_location_id']); ?>" style="width:100px;" />
							<input type="hidden" name="orig_spawn_location_id" value="<?php print($data['spawn_location_id']); ?>" />
						</td>
						<td>
							<input type="text" name="spawn_location_entry|spawnpercentage" value="<?php print($data['spawnpercentage']); ?>" style="width:100px;" />
							<input type="hidden" name="orig_spawnpercentage" value="<?php print($data['spawnpercentage']); ?>" />
							<input type="hidden" name="orig_condition" value="<?php print($data['condition']); ?>" />
						</td>
						<?php 
							$cond = $data['condition'];
							for ($i = 0; $i < 4; $i++) {
								$mask = 1 << $i;
								print("<td>");
								$eq2->GenerateBlueCheckbox("spawn_location_entry|condition|".$mask, $cond & $mask);
								print("</td>");
							}
						?>
						<td><?php if( $eq2->CheckAccess(G_DEVELOPER) ) { ?><input type="submit" name="cmd" value="Update" style="font-size:10px; width:60px" /><?php } ?></td>
	...
	&nbsp;
	<fieldset>
	<legend>Table: spawn_location_placement</legend>
	<table width="1100" cellpadding="5" border="0">
		<tr>
			<td valign="top">
				<?php $results = $spawns->GetSpawnLocationPlacements(); // moved here to get count() ?>
				<fieldset><legend>Location Data</legend>
				<table border="0" cellpadding="0" cellspacing="4">
		  <?php if( $eq2->CheckAccess(G_DEVELOPER) ) : ?>
          <tr>
		    <form method="post" id="sOffset">
          	<td colspan="10">Add X,Z Offsets to all spawn points for this spawn_id:
              <input type="submit" name="cmd" value="Sml (5px)" class="submit" />
              <input type="submit" name="cmd" value="Med (10px)" class="submit" />
              <input type="submit" name="cmd" value="Lrg (20px)" class="submit" />
              <input type="submit" name="cmd" value="None (0px)" class="submit" />
			  <input type="hidden" name="object_id" value="<?= $spawns->spawn_name ?>|<?= $spawns->spawn_id ?>" />
			  <input type="hidden" name="table_name" value="spawn_location_placement" />
            </td>
            <td colspan="5">&nbsp;</td>
			</form>
          </tr>
		  ...
		  <tr><td>
			<table class="ContrastTable">
					...
					<?php if( $eq2->CheckAccess(G_DEVELOPER) ) : ?>
					<form method="post" name="zsForm|new">
					<tr align="center">
						<td><strong>new</strong></td>
						<td><input type="text" name="spawn_location_placement|zone_id|new" value="<?= $_GET['zone'] ?>" style="width:40px;" /></td>
						<td><input type="text" name="spawn_location_placement|spawn_location_id|new" value="<?php echo $spawns->CreatedLocID ?>" style="width:70px;" /></td>
						<td><input type="text" name="spawn_location_placement|x|new" value="0" style="width:50px;" /></td>
						<td><input type="text" name="spawn_location_placement|y|new" value="0" style="width:50px;" /></td>
						<td><input type="text" name="spawn_location_placement|z|new" value="0" style="width:50px;" /></td>
						<td><input type="text" name="spawn_location_placement|x_offset|new" value="0" style="width:50px;" /></td>
						<td><input type="text" name="spawn_location_placement|y_offset|new" value="0" style="width:50px;" /></td>
						<td><input type="text" name="spawn_location_placement|z_offset|new" value="0" style="width:50px;" /></td>
						<td><input type="text" name="spawn_location_placement|heading|new" value="0" style="width:50px;" /></td>
						<td><input type="text" name="spawn_location_placement|pitch|new" value="0" style="width:50px;" /></td>
						<td><input type="text" name="spawn_location_placement|roll|new" value="0" style="width:50px;" /></td>
						<td><input type="text" name="spawn_location_placement|respawn|new" value="300" style="width:50px;" /></td>
						<td><input type="text" name="spawn_location_placement|expire_timer|new" value="0" style="width:50px;" /></td>
						<td><input type="text" name="spawn_location_placement|grid_id|new" value="0" style="width:90px;" /></td>
						<td colspan="2">
					...
					if( is_array($results) )
					{
						...
						<form method="post" name="naeForm" >
							...
							<td>
								<input type="text" name="spawn_location_placement|zone_id" value="<?php print($data['zone_id']); ?>" style="width:40px;" />
								<input type="hidden" name="orig_zone_id" value="<?php print($data['zone_id']); ?>" />
								<input type="hidden" name="object_id" value="<?= $spawns->spawn_name ?>|<?= $spawns->spawn_id ?>" />
								<input type="hidden" name="table_name" value="spawn_location_placement" />
							</td>
							<td>
								<input type="text" name="spawn_location_placement|spawn_location_id" value="<?php print($data['spawn_location_id']); ?>" style="width:70px;" />
								<input type="hidden" name="orig_spawn_location_id" value="<?php print($data['spawn_location_id']); ?>" />
							</td>
							<td>
								<input type="text" name="spawn_location_placement|x" value="<?php print($data['x']); ?>" style="width:50px;" />
								<input type="hidden" name="orig_x" value="<?php print($data['x']); ?>" />
							</td>
							<td>
								<input type="text" name="spawn_location_placement|y" value="<?php print($data['y']); ?>" style="width:50px;" />
								<input type="hidden" name="orig_y" value="<?php print($data['y']); ?>" />
							</td>
							<td>
								<input type="text" name="spawn_location_placement|z" value="<?php print($data['z']); ?>" style="width:50px;" />
								<input type="hidden" name="orig_z" value="<?php print($data['z']); ?>" />
							</td>
							<td>
								<input type="text" name="spawn_location_placement|x_offset" value="<?php print($data['x_offset']); ?>" style="width:50px;" />
								<input type="hidden" name="orig_x_offset" value="<?php print($data['x_offset']); ?>" />
							</td>
							<td>
								<input type="text" name="spawn_location_placement|y_offset" value="<?php print($data['y_offset']); ?>" style="width:50px;" />
								<input type="hidden" name="orig_y_offset" value="<?php print($data['y_offset']); ?>" />
							</td>
							<td>
								<input type="text" name="spawn_location_placement|z_offset" value="<?php print($data['z_offset']); ?>" style="width:50px;" />
								<input type="hidden" name="orig_z_offset" value="<?php print($data['z_offset']); ?>" />
							</td>
							<td>
								<input type="text" name="spawn_location_placement|heading" value="<?php print($data['heading']); ?>" style="width:50px;" />
								<input type="hidden" name="orig_heading" value="<?php print($data['heading']); ?>" />
							</td>
							<td>
								<input type="text" name="spawn_location_placement|pitch" value="<?php print($data['pitch']); ?>" style="width:50px;" />
								<input type="hidden" name="orig_pitch" value="<?php print($data['pitch']); ?>" />
							</td>
							<td>
								<input type="text" name="spawn_location_placement|roll" value="<?php print($data['roll']); ?>" style="width:50px;" />
								<input type="hidden" name="orig_roll" value="<?php print($data['roll']); ?>" />
							</td>
							<td>
								<input type="text" name="spawn_location_placement|respawn" value="<?php print($data['respawn']); ?>" style="width:50px;" />
								<input type="hidden" name="orig_respawn" value="<?php print($data['respawn']); ?>" />
							</td>
							<td>
								<input type="text" name="spawn_location_placement|expire_timer" value="<?php print($data['expire_timer']); ?>" style="width:50px;" />
								<input type="hidden" name="orig_expire_timer" value="<?php print($data['expire_timer']); ?>" />
							</td>
							<td>
								<input type="text" name="spawn_location_placement|grid_id" value="<?php print($data['grid_id']); ?>" style="width:90px;" />
	...
	<table class="ContrastTable">
		...
		<tr align="center">
		<form method="post" name="slpOverrideForm">
			<td width="30">
				<strong><?php print($data['id']); ?></strong>
				<input type="hidden" name="orig_id" value="<?php print($data['id']); ?>" />
			</td>
			<td>
				<input type="text" name="spawn_location_placement|lvl_override" value="<?php print($data['lvl_override']); ?>" style="width:30px;" />
				<input type="hidden" name="orig_lvl_override" value="<?php print($data['lvl_override']); ?>" />
			</td>
			<td>
				<input type="text" name="spawn_location_placement|difficulty_override" value="<?php print($data['difficulty_override']); ?>" style="width:30px;" />
				<input type="hidden" name="orig_difficulty_override" value="<?php print($data['difficulty_override']); ?>" />
			</td>
			<td>
				<input type="text" name="spawn_location_placement|hp_override" value="<?php print($data['hp_override']); ?>" style="width:90px;" />
				<input type="hidden" name="orig_hp_override" value="<?php print($data['hp_override']); ?>" />
			</td>
			<td>
				<input type="text" name="spawn_location_placement|mp_override" value="<?php print($data['mp_override']); ?>" style="width:50px;" />
				<input type="hidden" name="orig_mp_override" value="<?php print($data['mp_override']); ?>" />
			</td>
			<td>
				<input type="text" name="spawn_location_placement|str_override" value="<?php print($data['str_override']); ?>" style="width:50px;" />
				<input type="hidden" name="orig_str_override" value="<?php print($data['str_override']); ?>" />
			</td>
			<td>
				<input type="text" name="spawn_location_placement|sta_override" value="<?php print($data['sta_override']); ?>" style="width:50px;" />
				<input type="hidden" name="orig_sta_override" value="<?php print($data['sta_override']); ?>" />
			</td>
			<td>
				<input type="text" name="spawn_location_placement|wis_override" value="<?php print($data['wis_override']); ?>" style="width:50px;" />
				<input type="hidden" name="orig_wis_override" value="<?php print($data['wis_override']); ?>" />
			</td>
			<td>
				<input type="text" name="spawn_location_placement|int_override" value="<?php print($data['int_override']); ?>" style="width:50px;" />
				<input type="hidden" name="orig_int_override" value="<?php print($data['int_override']); ?>" />
			</td>
			<td>
				<input type="text" name="spawn_location_placement|agi_override" value="<?php print($data['agi_override']); ?>" style="width:50px;" />
				<input type="hidden" name="orig_agi_override" value="<?php print($data['agi_override']); ?>" />
			</td>
			<td>
				<input type="text" name="spawn_location_placement|heat_override" value="<?php print($data['heat_override']); ?>" style="width:50px;" />
				<input type="hidden" name="orig_heat_override" value="<?php print($data['heat_override']); ?>" />
			</td>
			<td>
				<input type="text" name="spawn_location_placement|cold_override" value="<?php print($data['cold_override']); ?>" style="width:50px;" />
				<input type="hidden" name="orig_cold_override" value="<?php print($data['cold_override']); ?>" />
			</td>
			<td>
				<input type="text" name="spawn_location_placement|magic_override" value="<?php print($data['magic_override']); ?>" style="width:50px;" />
				<input type="hidden" name="orig_magic_override" value="<?php print($data['magic_override']); ?>" />
			</td>
			<td>
				<input type="text" name="spawn_location_placement|mental_override" value="<?php print($data['mental_override']); ?>" style="width:50px;" />
				<input type="hidden" name="orig_mental_override" value="<?php print($data['mental_override']); ?>" />
			</td>
			<td>
				<input type="text" name="spawn_location_placement|divine_override" value="<?php print($data['divine_override']); ?>" style="width:50px;" />
				<input type="hidden" name="orig_divine_override" value="<?php print($data['divine_override']); ?>" />
			</td>
			<td>
				<input type="text" name="spawn_location_placement|disease_override" value="<?php print($data['disease_override']); ?>" style="width:50px;" />
				<input type="hidden" name="orig_disease_override" value="<?php print($data['disease_override']); ?>" />
			</td>
			<td>
				<input type="text" name="spawn_location_placement|poison_override" value="<?php print($data['poison_override']); ?>" style="width:50px;" />
				<input type="hidden" name="orig_poison_override" value="<?php print($data['poison_override']); ?>" />
			</td>
			<?php if ($eq2->CheckAccess(G_DEVELOPER)) : ?>
				<td>
				<input type="hidden" name="table_name" value="spawn_location_placement" />
...
?>
	<table border="0" cellpadding="5">
		<tr>
			<td width="600" valign="top">
				<fieldset><legend>merchants</legend>
				...
				<table width="100%" cellpadding="0" border="0">
					...
					<?php
					$query = sprintf("select * from `%s`.merchants where merchant_id = %s",	DEV_DB, $id);
	...
	<table border="0" cellpadding="5">
		<tr>
			<td valign="top">
				<fieldset><legend>merchant_inventory</legend>
				<table width="100%" cellpadding="0" border="0">
					<tr>
						<td width="55">id</td>
						<td width="75">inventory_id</td>
					...
					$query=sprintf("select * from `%s`.%s where inventory_id = %s", DEV_DB, $table, $inventory_id);
					...
					<tr>
						...
						<td>
							...
							<span style="font-size:9px; vertical-align:middle; text-align:left"><?php print($eq2->getItemName($data['item_id'])); ?></span>
		...
		<tr>
			<td width="1000" valign="top">
				...
				<table width="100%" cellpadding="0" border="0">
					...
					$query=sprintf("select * from `%s`.%s where merchant_id = %s", DEV_DB, $table, $id);
					$result = $eq2->RunQueryMulti($query);
					foreach ($result as $data) {
					?>
